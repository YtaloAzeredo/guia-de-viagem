...
🎊 SUA RESPOSTA ESPECIALIZADA:""",
        ...
        # Cria a chain
        self.chain = self.local_info_template | self.llm | StrOutputParser()
    ...
    def get_local_info(self, route_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "city": main_city,
                "places_found": (
                    len(relevant_places.split("---")) if relevant_places else 0
                ),
            }
    ...
    def _search_relevant_places(self, query: str, city: str) -> str:
        ...
        if results:
            ...
            return (
                self._format_places_info(results)
                if results
                else "Nenhuma informação específica encontrada."
            )
    ...
    def _get_additional_context(self, query: str, city: str) -> str:
        ...
        if city:
            ...
            if city_overview:
                categories = set(
                    place.get("categoria", "") for place in city_overview[:5]
                )
                context_parts.append(
                    f"Categorias disponíveis em {city}: {', '.join(filter(None, categories))}"
                )
        ...
        if any(
            word in query_lower for word in ["museu", "cultura", "arte", "história"]
        ):
        ...
        return (
            "\n".join(context_parts)
            if context_parts
            else "Consulta geral sobre informações locais."
        )
