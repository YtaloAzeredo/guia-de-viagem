...
class RAGSystem:
    ...
    def load_and_index_data(self, force_reload: bool = False):
        ...
        if not force_reload:
            try:
                ...
                if stats.total_vector_count > 0:
                    print(
                        f"Índice já contém {stats.total_vector_count} vetores. Use force_reload=True para recarregar."
                    )
                    return
            except Exception as e:
        ...
        for item in all_data:
            ...
            vector = {"id": item["id"], "values": embedding, "metadata": item}
    ...
    def _search_local(
        self, query: str, top_k: int, filter_dict: Dict = None
    ) -> List[Dict]:
